  ...
  handleSelection(interactives) {
    this.setState((prevState) => {
      const state = toObject(interactives, (each) => {
        return [
          each.type === "FibonacciRetracement" ? "retracements" : "trends",
          each.objects,
        ];
      });

      const fibs = state.retracements || [];
...
{selectedTrendIndex !== null && (
  ...
  trends={trends_1.map(t => ({
    ...t,
    appearance: { stroke: t.stroke || '#3d83b5', strokeWidth: t.strokeWidth || 1 } // se a propriedade strokeWidth estiver presente, use-a, senão, use 1
  }))}
